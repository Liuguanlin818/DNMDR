...
def med_process(med_file):
    ...
    med_pd["ENDDATE"] = pd.to_datetime(
        med_pd["ENDDATE"], format="%Y-%m-%d %H:%M:%S"
    )
    med_pd.sort_values(
...
def atc3toSMILES(ATC3toDrugDict, druginfo):  # atc3到SMILES string
    drug2smiles = {}
    atc3tosmiles = {}
...
def codeMapping2atc4(med_pd):
    ...
    RXCUI2atc4.drop_duplicates(subset=["RXCUI"], inplace=True)  # 去重
    med_pd.drop(index=med_pd[med_pd["RXCUI"].isin([""])].index, axis=0, inplace=True)  # 删掉rxcui不存在的

    med_pd["RXCUI"] = med_pd["RXCUI"].astype("int64")  # 类型转换
    med_pd = med_pd.reset_index(drop=True)  # 冲编码
    med_pd = med_pd.merge(RXCUI2atc4, on=["RXCUI"])  # 两表合并
    med_pd.drop(columns=["NDC", "RXCUI"], inplace=True)  # 删掉NDC和RXCUI
    med_pd["ATC4"] = med_pd["ATC4"].map(lambda x: x[:4])  #
    med_pd = med_pd.rename(columns={"ATC4": "ATC3"})
    med_pd = med_pd.drop_duplicates()  # 去重
    med_pd = med_pd.reset_index(drop=True)  # 重编码
    return med_pd
...
def getgraph(data):
    diag_pd = data["ICD9_CODE"]
    pro_pd = data["PRO_CODE"]
    med_pd = data["ATC3"]
    Start_pd = data["STARTDATE"]
    end_pd = data["ENDDATE"]
    maxicd9 = 0
    minicd9 = 1000000000000000
    for i in diag_pd:
        for j in i:
            if j > maxicd9:
                maxicd9 = j
...
def combine_process(med_pd, diag_pd, pro_pd):
    ...
    med_startdata = med_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["STARTDATE"].unique().reset_index()
    med_enddata = med_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["ENDDATE"].unique().reset_index()
    med_pd = med_pd.groupby(by=["SUBJECT_ID", "HADM_ID"])["ATC3"].unique().reset_index()
    med_pd = med_pd.merge(med_startdata, on=["SUBJECT_ID", "HADM_ID"], how="inner").reset_index()
    med_pd = med_pd.merge(med_enddata, on=["SUBJECT_ID", "HADM_ID"], how="inner").reset_index()
    pro_pd = (
...
def statistics(data):  # 统计数据
    ...
    (
        ...
        max_visit,
        avg_visit,
    ) = [0 for i in range(9)]  # 定义初始是0

    for subject_id in data["SUBJECT_ID"].unique():  # 列举subjectid
        item_data = data[data["SUBJECT_ID"] == subject_id]  # 对应subject的数据
        x, y, z = [], [], []
        visit_cnt = 0
...
class Voc(object):
    ...
    def add_sentence(self, sentence):  # 添加
...
def create_str_token_mapping(df):
    ...
    dill.dump(
        ...
        obj={"diag_voc": diag_voc, "med_voc": med_voc, "pro_voc": pro_voc},  # 保存到词汇文件
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    ...
    dill.dump(obj=records, file=open(ehr_sequence_file, "wb"))  # 讲病例写入文件
    return records


# get ddi matrix
def get_ddi_matrix(records, med_voc, ddi_file):  # ddi矩阵
    ...
    with open(cid2atc6_file, "r") as f:  # drug-atc文件
        for line in f:
            line_ls = line[:-1].split(",")
            cid = line_ls[0]  # cid
            atcs = line_ls[1:]  # atc码
            for atc in atcs:  # 一个cid对应多个atc码
                if len(atc3_atc4_dic[atc[:4]]) != 0:  # 如果词典有，就把他加入
...
if __name__ == "__main__":
    ...
    med_file = "/root/autodl-tmp/SafeDrug-main/data/input/PRESCRIPTIONS.csv"
    diag_file = "/root/autodl-tmp/SafeDrug-main/data/input/DIAGNOSES_ICD.csv"
    procedure_file = (
        "/root/autodl-tmp/SafeDrug-main/data/input/PROCEDURES_ICD.csv"
    )
    # input auxiliary files
    med_structure_file = "/root/autodl-tmp/SafeDrug-main/data/output/atc32SMILES.pkl"
    RXCUI2atc4_file = "/root/autodl-tmp/SafeDrug-main/data/input/RXCUI2atc4.csv"
    cid2atc6_file = "/root/autodl-tmp/SafeDrug-main/data/input/drug-atc.csv"
    ndc2RXCUI_file = "/root/autodl-tmp/SafeDrug-main/data/input/ndc2RXCUI.txt"
    ddi_file = "/root/autodl-tmp/SafeDrug-main/data/input/drug-DDI.csv"
    drugbankinfo = "/root/autodl-tmp/SafeDrug-main/data/input/drugbank_drugs_info.csv"

    # output files
    ddi_adjacency_file = "/root/autodl-tmp/SafeDrug-main/data/output/ddi_A_final.pkl"
    ehr_adjacency_file = "/root/autodl-tmp/SafeDrug-main/data/output/ehr_adj_final.pkl"
    ehr_sequence_file = "/root/autodl-tmp/SafeDrug-main/data/output/records_final.pkl"
    vocabulary_file = "/root/autodl-tmp/SafeDrug-main/data/output/voc_final.pkl"
    ddi_mask_H_file = "/root/autodl-tmp/SafeDrug-main/data/output/ddi_mask_H.pkl"
    atc3toSMILES_file = "/root/autodl-tmp/SafeDrug-main/data/output/atc3toSMILES.pkl"
