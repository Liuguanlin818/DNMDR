...
np.random.seed(2048)
model_name = "SafeDrug"
resume_path = "Epoch_49_TARGET_0.06_JA_0.5112_DDI_0.062.model"
...
def eval(model, data_eval, voc_size, epoch):
    ...
    # ddi rate
    ddi_rate = ddi_rate_score(smm_record, path="/root/autodl-tmp/SafeDrug-main/data/output/ddi_A_final.pkl")
...
# 用于计算患者的诊断和治疗之间的条件概率，并将结果保存在文件中。
# train_key_set: 可选参数，是一个集合（set），表示训练集的患者ID（encounter_id）
# 如果提供了train_key_set，则只会对训练集的数据进行统计和计算，否则将对所有数据进行统计和计算。
def count_conditional_prob_dp(seqex_list, output_path, train_key_set=None):
    dx_freqs = {}  # 诊断的频率字典，用于统计每个诊断的出现次数
    proc_freqs = {}  # 程序的频率字典，用于统计每个诊断的出现次数
    med_freqs = {}  # 治疗的频率字典，用于统计每个治疗的出现次数
    dm_freqs = {}  # 诊断-治疗对的频率字典，用于统计每个诊断-治疗对的出现次数
    pm_freqs = {}  # 程序-治疗对的频率字典，用于统计每个诊断-治疗对的出现次数
    total_visit = 0  # 总的访问次数（包括在训练集中的和不在训练集中的数据）
    for seqex in seqex_list:
        ...
        # key = seqex
        # 如果所得的key（该病人记录）不在训练集中则total_visit + len(key)（该病人记录的所有访问次数）
        if train_key_set is not None and seqex not in train_key_set:
            total_visit += len(seqex)
            continue

        for key in seqex:
            # 取该病人的诊断记录和治疗记录分别置于dx_ids和proc_ids中
            dx_ids = key[0]
            proc_ids = key[1]
            med_ids = key[2]
            ...
            # 依次取一条诊断记录
            for dx in dx_ids:
                # 若该诊断记录不在总的诊断记录频率统计字典中，则先将其初始化为0
    ...
    dm_cond_probs = {}  # 存储诊断-治疗对的条件概率，键为诊断-治疗对，值为对应的条件概率
    md_cond_probs = {}  # 存储治疗-诊断对的条件概率，键为治疗-诊断对，值为对应的条件概率
    for dx, dx_prob in dx_probs.items():
        for med, med_prob in med_probs.items():
            ...
            # 若dp不在dp_probs中，表示该诊断-治疗对在数据中还没有出现过，也就是说它的频率为零。
            # 在这种情况下，为了避免除零错误，可以将条件概率设置为零。
            else:
                dm_cond_probs[dm] = 0.0
                md_cond_probs[md] = 0.0

    pm_cond_probs = {}  # 存储程序-治疗对的条件概率，键为程序-治疗对，值为对应的条件概率
    mp_cond_probs = {}  # 存储治疗-程序对的条件概率，键为治疗-程序对，值为对应的条件概率
...
# 将之前计算的条件概率作为先验信息，添加到患者的记录中，以帮助后续的机器学习模型进行训练和预测。
# max_num_codes：一个整数，表示最大的诊断和治疗编码数。默认为 50。
def add_sparse_prior_guide_dp(seqex_list, stats_path, key_set=None):
    # 加载存储在 stats_path 中的条件概率信息
    ...
    total_visit = 0
    # 定义一个用于存储添加了先验指导信息的 SequenceExample 对象的新列表
    new_seqex_list = []
    ...
    for seqex in seqex_list:
        ...
        for key in seqex:
            ...
            # 构建稀疏的先验指导矩阵
            indices_dpm = []  # 存储稀疏矩阵非零元素的索引
            values_dpm = []  # 存储稀疏矩阵非零元素的值

            # 遍历 诊断-药物 编码的组合，计算相应的条件概率，并添加到稀疏矩阵中
            # enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标，一般用在 for 循环当中。
            for i, dx in enumerate(dx_ids):
                for j, med in enumerate(med_ids):
                    dm = str(dx) + ',' + str(med)
                    indices_dpm.append((i, len(dx_ids) + len(proc_idx) + med))  # 诊断 i 和治疗 j 对应的索引位置
                    prob = 0.0 if dm not in dm_cond_probs else dm_cond_probs[dm]  # 获取 dp_cond_probs 中的条件概率值
                    values_dpm.append(prob)

            # 遍历 程序-药物 编码的组合，计算相应的条件概率，并添加到稀疏矩阵中
            for i, proc in enumerate(proc_idx):
                for j, med in enumerate(med_ids):
                    pm = str(proc) + ',' + str(med)
                    indices_dpm.append((len(dx_ids) + i, len(dx_ids) + len(proc_idx) + med))  # 诊断 i 和治疗 j 对应的索引位置
                    prob = 0.0 if pm not in pm_cond_probs else pm_cond_probs[pm]  # 获取 dp_cond_probs 中的条件概率值
                    values_dpm.append(prob)

            # 遍历 药物-诊断-程序 编码的组合，计算相应的条件概率，并添加到稀疏矩阵中
...
def main():
    ...
    data_train = data[:split_point]  # 1~2/3
