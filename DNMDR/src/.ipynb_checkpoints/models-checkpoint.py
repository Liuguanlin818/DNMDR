...
class SafeDrugModel(nn.Module):
    def __init__(
        ...
        for embedding_layer in self.embeddings:  # 对embedding.weights进行初始化，否则容易出现NAN的情况
        ...
        self.ehr_gcn = GraphAT(voc_size=vocab_size[2], emb_dim=emb_dim, adj=ehr_adj, device=device)
        self.ddi_gcn = GraphAT(voc_size=vocab_size[2], emb_dim=emb_dim, adj=ddi_adj, device=device)
        self.inter = nn.Parameter(torch.FloatTensor(1))
        # self.w4 = nn.Parameter(torch.FloatTensor(1))
        ...
        self.d_multi_head_attention = nn.MultiheadAttention(embed_dim=64, num_heads=4)  # 创建多头注意力模型
        self.p_multi_head_attention = nn.MultiheadAttention(embed_dim=64, num_heads=4)  # 创建多头注意力模型
    ...
    def forward(self, input):
        ...
        if len(input) > 1:
            ...
            dpm_embs = egcn(hist_dpm_adj_list, hist_dpm_ndFeats_list)  # (|D+P+M|,dim)
            '''dpm_embs = []
            for t, Ahat in enumerate(hist_dpm_adj_list):
                node_embs = hist_dpm_ndFeats_list[t]
                node_embs = self.gcn(node_embs, Ahat)  # (|D+P+M|,dim)
                dpm_embs.append(node_embs)'''
            ...
            for i, embs in enumerate(dpm_embs):
                '''
                d_embs.append(embs[:len(input[i][0])].sum(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))  # (1,1,dim)
                p_embs.append(embs[len(input[i][0]):len(input[i][0]) + len(input[i][1])].sum(dim=0).unsqueeze(dim=0).unsqueeze(dim=0))  # (1,1,dim)
                '''
                di_embs = embs[:len(input[i][0])].unsqueeze(dim=0)  #.permute(1,0,2)  # (1,|D|,dim)
                ...
                # d_embs.append(d_output.permute(1,0,2).sum(dim=1, keepdim=True))

                pi_embs = embs[len(input[i][0]):len(input[i][0]) + len(input[i][1])].unsqueeze(dim=0)  # .permute(1,0,2)  # (1,|P|,dim)
                ...
                # p_embs.append(p_output.permute(1, 0, 2).sum(dim=1, keepdim=True))

                m_embs.append(embs[-self.vocab_size[2]:].unsqueeze(dim=0))  # (1,|M|,dim)

            def sum_embedding(embedding):
                return embedding.sum(dim=1).unsqueeze(dim=0)  # (1,seq,dim) -> (1,1,dim)
            ...
            '''
            i1 = self.dropout(self.embeddings[0](torch.LongTensor(input[-1][0]).unsqueeze(dim=0).to(self.device)))
            i1_output, i1_output_weight = self.d_multi_head_attention(i1, i1, i1)
            i1 = i1_output.mul(i1_output_weight.permute(1, 0, 2)).sum(dim=1, keepdim=True)
            i2 = self.dropout(self.embeddings[1](torch.LongTensor(input[-1][1]).unsqueeze(dim=0).to(self.device)))
            i2_output, i2_output_weight = self.p_multi_head_attention(i2, i2, i2)
            i2 = i2_output.mul(i2_output_weight.permute(1, 0, 2)).sum(dim=1, keepdim=True)'''

            d_embs.append(i1)
            p_embs.append(i2)
            d_embs = torch.cat(d_embs, 1)  # (t,dim)
            p_embs = torch.cat(p_embs, 1)  # (t,dim)
            o1, h1 = self.encoders[0](d_embs)  # o1:(1, t, dim*2)
            o2, h2 = self.encoders[1](p_embs)
            m_embs, h3 = self.m_encoders[0](m_embs[-1])
            patient_representations = torch.cat([o1, o2], dim=-1).squeeze(dim=0)  # (t, dim*4)
            ...
            for adm in input:
                ...
                i1_seq.append(i1)
                i2_seq.append(i2)
            i1_seq = torch.cat(i1_seq, dim=1)  # seq个(1,1,dim) -> (1,seq,dim)
            i2_seq = torch.cat(i2_seq, dim=1)  # (1,seq,dim)
            o1, h1 = self.encoders[0](i1_seq)  # o1:(1, seq, dim*2)
            o2, h2 = self.encoders[1](i2_seq)
            patient_representations = torch.cat([o1, o2], dim=-1).squeeze(dim=0)  # (t, dim*4)
        queries = self.query(patient_representations)  # (t, dim)
        query = queries[-1:]  # (1,dim)


        #bipartite_weights = self.mask_H_transform(self.tensor_ddi_mask_H)  # (|M|,dim)
        #drug_embedding = self.MPNN_emb + bipartite_weights * self.w6

        # ①生成图记忆 MB
        drug_memory = self.ehr_gcn() - self.ddi_gcn() * self.inter  # (|M|, dim)
        key_weights = F.sigmoid(torch.mm(query, drug_memory.t()))  # (1, |M|)
        key_att = self.key_layernorm(key_weights + self.key_output(key_weights))
        # fact1 = query + torch.mm(key_weights, drug_memory) * self.w4  # (1, dim)
        fact1 = query

        if len(input) > 1:
            m_embs = m_embs.squeeze(0)  # (|M|,dim)
            history_key = F.sigmoid(torch.mm(query, m_embs.t()))  # (1, |M|)
            history_value = torch.mm(history_key, m_embs)  # (1,dim)
            fact1 = fact1 + history_value * self.w5

        # MPNN embedding
        MPNN_match = F.sigmoid(torch.mm(fact1, self.MPNN_emb.t()))  # (1,|M|)
        MPNN_att = self.MPNN_layernorm(MPNN_match + self.MPNN_output(MPNN_match))  # (1,|M|)

        # local embedding
        bipartite_emb = self.bipartite_output(F.sigmoid(self.bipartite_transform(query)),self.tensor_ddi_mask_H.t())  # (1,|M|)

        result = self.output(torch.cat([query, fact1], dim=-1))  # (1, dim)
        # result = torch.mul(key_weights, MPNN_att)  # （1，|M|）
